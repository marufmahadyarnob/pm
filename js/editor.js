...
const templateImg=new Image(); templateImg.src=template.image;

// Image State
let userImg=null,imgX=frame.x,imgY=frame.y,scale=1,dragging=false,startX=0,startY=0;
...
// Image Draw
function drawImage(){if(!userImg)return;
ctx.save(); ctx.beginPath();
ctx.rect(frame.x,frame.y,frame.w,frame.h); ctx.clip();
...
function wrapText(text,x,y,maxWidth,lineHeight){
  const words=text.split(" "),lines=[],ctx2=ctx;
  let line="";
  for(let w of words){
    const test=line+w+" ";
    if(ctx2.measureText(test).width>maxWidth){lines.push(line);line=w+" ";}else line=test;
  }
  lines.push(line);
...
function drawText(){
  ...
  ctx.font=`bold 48px ${font.value}`; ctx.fillStyle=textColor.value; ctx.textAlign=align.value;
  const x=align.value==="left"?80:align.value==="right"?1000:540;
  const y=parseInt(textY.value);
  const maxW=900;
  if(banner.checked){ctx.fillStyle=bannerColor.value; ctx.fillRect(0,y-50,1080,180); ctx.fillStyle=textColor.value;}
  let offset=0;
  blocks.forEach(b=>{
    if(!b) return;
...
// Controls
upload.onchange=e=>{const img=new Image(); img.src=URL.createObjectURL(e.target.files[0]); img.onload=()=>{userImg=img;saveState();draw();}};
zoom.oninput=e=>{scale=e.target.value;draw();};
rotate.oninput=draw; text1.oninput=draw; text2.oninput=draw; text3.oninput=draw;
...
// Reset
reset.onclick=()=>{imgX=frame.x;imgY=frame.y;scale=1;rotate.value=0;text1.value=text2.value=text3.value="";draw(); saveState();};
...
// Download
download.onclick=()=>{const type=format.value==="jpg"?"image/jpeg":"image/png";
const link=document.createElement("a"); link.download=`photocard.${format.value}`; link.href=canvas.toDataURL(type,0.95); link.click();}
